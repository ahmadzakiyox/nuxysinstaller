...
usage_and_exit() {
    if is_in_windows; then
        reinstall_____='.\avl.bat'
    else
        reinstall_____=' ./avl.sh'
    fi
    cat <<EOF
...
prompt_password() {
    info "Setting password automatically"
    password="Avola123#"
    password_confirm="Avola123#"
...
build_extra_cmdline() {
    ...
    for key in confhome hold force force_cn force_old_windows_setup cloud_image main_disk \
        elts deb_mirror \
        ssh_port id_install user_p rdp_port pas_rdp web_port allow_ping; do
        value=${!key}
        if [ -n "$value" ]; then
            is_need_quote "$value" &&
...
echo "System will reboot automatically in 30 seconds..."
sleep 30
reboot
